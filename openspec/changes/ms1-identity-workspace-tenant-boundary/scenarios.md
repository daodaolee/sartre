...
| --- | --- | --- | --- |
| positive-human | Given an operator-provisioned local account, when password login completes, then one Human Session and short-lived access identity are created without exposing credentials. | REAL_TEST | `none` |
| login-rejection | Given a missing account or wrong password, when login runs, then identity existence is not disclosed and no session is issued. Public registration is absent. | REAL_TEST | `authentication_failed` |
...
| dependency-failure | Given database/Hub/Runtime is unavailable or degraded, when the relevant flow runs, then UI/API names the stable failure and recovery action without false success. | REAL_TEST | `dependency_unavailable` |
