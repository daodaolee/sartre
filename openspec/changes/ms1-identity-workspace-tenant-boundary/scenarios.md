...
| --- | --- | --- | --- |
| positive-human | Given an approved company Human, when verified-email auth completes, then one Human Session and short-lived access identity are created without exposing credentials. | REAL_TEST | `none` |
...
| dependency-failure | Given mail/database/Hub/Runtime is unavailable or degraded, when the relevant flow runs, then UI/API names the stable failure and recovery action without false success. | REAL_TEST | `dependency_unavailable` |
