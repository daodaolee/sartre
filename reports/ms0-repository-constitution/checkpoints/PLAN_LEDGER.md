...
- Status: IN_PROGRESS
- Current task: Task 5 is DONE at reviewed subject `616b2c0e5be133622f03405c2db746c31a8e6ee5`. Targeted specification re-review approved the final lazy-registry repair, and the same code-quality reviewer reported Critical `0`, Important `0`, Ready `YES`. Task 6 has not started, MS0 remains IN_PROGRESS, and MS1 remains out of scope
- Last verified action: the targeted specification reviewer freshly ran `pnpm exec vitest run tests/integration/service-health.integration.test.ts` at subject `616b2c0e5be133622f03405c2db746c31a8e6ee5` and observed `1 file | 10 tests passed`; the same quality reviewer then approved the reviewed subject with no open Critical or Important findings
- Evidence level: Task 5 process-health REAL_TEST, one-time root/static matrix, immutable-index/full repository Secret checks, and artifact Secret checks are PASS; `architecture:check` remains STRUCTURAL_CHECK only. Task 5 dual review is closed, but no Task 6 or MS0 closeout claim is made
...
- Resume procedure:
  ...
  2. Run `git status --short --branch` and `git log -5 --format='%H %P %s'`. Expect branch `codex/ms0-repository-constitution`, a clean five-commit chain, Task 5 subject `feat(ms0): add service health process shells`, and sole parent `d5d370cd4b1378ec022233208a1debcd7dab2d03`. Never revert Task 1-5.
  3. Begin Task 6 Step 1 only by creating the failing monitor, packaging-policy, and Electron E2E tests. Before writing React/Electron production code, use `vercel-react-best-practices`. The first verification command is `pnpm exec vitest run apps/electron-app/src/main/health/health-monitor.test.ts apps/electron-app/src/main/packaging-policy.test.ts`; require discovered behavioral failures caused by missing Task 6 behavior, not missing files, zero discovery, syntax/dependency failure, or SKIPPED assertions.
  4. Do not rerun Task 5's full root/static matrix unless Task 5 source changes. Do not begin MS1, push, or regenerate the legacy freeze manifest.
